...
> **Estado (2026-09-22): implementado.** Las 9 tareas y la adenda de Colab están hechas. Hay dos corridas evaluadas:
>
> - v1: 0.791 de macro-F1 de familia en prueba.
> - v2: 0.832, con medidas contra el sobreajuste.
>
> La v3 (EfficientNet-B2 a 288 px) está entrenando. Resultados y siguientes pasos: `CLAUDE.md` y `docs/RESUMEN_EJECUTIVO.md`.
>
> Cambios respecto del texto original:
>
> - Checkpoint reanudable, precisión mixta y tasa en coseno.
> - Aumentos más fuertes y suavizado de etiquetas.
> - Resolución configurable, que exportar y evaluar leen de `config.json`.
> - Paridad ONNX juzgada con fotos reales y probabilidades.
> - Análisis por clase en `pipeline/desempeno.py`.
...
## Adenda (2026-09-19): entrenar en Colab gratis

**Decisión del proyecto:** el entrenamiento se hace en Google Colab gratuito (GPU T4), con **una sola cuenta**. La primera versión de esta adenda contemplaba turnar dos cuentas sobre una carpeta compartida; el equipo lo descartó el mismo día y se retiró el arbitraje de turnos.

**Por qué igual hace falta reanudar.** Colab gratuito corta sesiones sin aviso: por inactividad, por agotar la cuota diaria de GPU o por un límite de duración. Sin checkpoint, cada corte hace perder todo el entrenamiento. Con un checkpoint por época, se pierde como mucho una época, y se continúa en la sesión siguiente, ese mismo día o al otro.

**Estimación de tiempo.** Con 24 032 imágenes de entrenamiento, EfficientNet-B0 a 224 px en una T4 con precisión mixta tarda unos 2 a 4 minutos por época, siempre que las imágenes estén en el disco local de Colab y no se lean desde Drive. Las 25 épocas caben probablemente en una sola sesión.

### Estructura en Google Drive (de la cuenta que entrena)
...
└─ corridas/
   └─ <nombre_corrida>/
      ...
      ├─ metricas.json     historial y resultado al terminar
      ├─ etiquetas.json    espacio de clases
      └─ config.json       hiperparámetros (se fijan al crear la corrida)
```

### Cambios sobre las tareas del plan

**Task 6 (entrenar.py): checkpoint completo y reanudación.** Implementado en `41ffd5f`.

- `ultimo.pth` se escribe al cerrar cada época, de forma atómica. Guarda:
  - los pesos, el optimizador y el escalador de precisión mixta;
  - el historial, la mejor métrica y la época en que se logró;
  - el estado de los generadores aleatorios.
- Con `--reanudar`:
  - Continúa desde la época siguiente.
  - Si no hay checkpoint, empieza de cero; por eso el cuaderno siempre pasa la opción.
  - Si la corrida ya terminó, no vuelve a entrenar.
- Protecciones:
  - Sin `--reanudar`, no se pisa una corrida existente.
  - Con otra configuración, la reanudación falla.
- Precisión mixta en GPU.
- Pruebas en `tests/test_entrenar_reanudar.py`:
  - Cortar y reanudar da el mismo historial que no cortar, con el corte en la frontera entre la fase congelada y la descongelada.
  - Un corte durante la escritura deja sano el checkpoint anterior.

**Task 9 (nueva): empaquetado y cuaderno de Colab.**

- `pipeline/empaquetar.py` arma `dataset_v1.zip` y escribe su sha256. El zip lleva:
  - `train.csv`, `val.csv` y `test.csv`;
  - `clases.yaml`;
  - **solo** las imágenes que esos CSV referencian.

  Antes de empaquetar verifica que cada imagen referenciada exista.
- `colab/entrenar.ipynb`:
  1. Monta Drive.
  2. Clona el repositorio.
  3. Instala las dependencias.
  4. Copia `dataset_v1.zip` a `/content`, verifica el sha256 y lo descomprime.
  5. Corre `python -m pipeline.entrenar --reanudar --destino <Drive>/corridas/<corrida>`.

  Para continuar tras un corte, se vuelven a correr todas las celdas.
